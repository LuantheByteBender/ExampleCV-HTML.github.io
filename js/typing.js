...
if (roleElement && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
  ...
  const type = () => {
    ...
    if (!deleting && characterIndex === role.length) { deleting = true; window.setTimeout(type, 1800); return; }
    if (deleting && characterIndex === 0) { deleting = false; roleIndex = (roleIndex + 1) % roles.length; window.setTimeout(type, 260); return; }
